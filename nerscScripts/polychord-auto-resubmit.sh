#!/bin/bash
#
# Auto-resubmit wrapper for preemptible PolyChord jobs
# This script monitors the job and resubmits if preempted
#
# Usage: ./polychord-auto-resubmit.sh [max_resubmits]
#   max_resubmits: maximum number of times to resubmit (default: 10)
#
# The script checks for convergence by looking for the .resume file
# and the final evidence output in the chain file.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BATCH_SCRIPT="${SCRIPT_DIR}/cosmosis-batch-polychord-preempt.sh"
CHAIN_DIR="${PSCRATCH}/chains/winter2025/y3cpp/chains"
POLYCHORD_OUTPUT="${CHAIN_DIR}/polychord_output"
...
echo "=========================================="
echo "PolyChord Auto-Resubmit Wrapper"
echo "=========================================="
echo "Max resubmits: ${MAX_RESUBMITS}"
echo "Batch script: ${BATCH_SCRIPT}"
echo "Output dir: ${POLYCHORD_OUTPUT}"
echo ""

# Function to check if PolyChord has converged
check_convergence() {
    # Check if the .stats file exists and contains final evidence
    local stats_file="${POLYCHORD_OUTPUT}/polychord_full.stats"
    if [[ -f "${stats_file}" ]]; then
        if grep -q "log(Z)" "${stats_file}" 2>/dev/null; then
            echo "Convergence detected in ${stats_file}"
            return 0
        fi
    fi
    return 1
...
while [[ ${RESUBMIT_COUNT} -lt ${MAX_RESUBMITS} ]]; do
    ...
    if check_convergence; then
        echo "PolyChord has converged! Exiting."
        exit 0
    fi
    ...
    while true; do
        ...
        if [[ -z "${STATE}" ]]; then
            ...
            if [[ "${JOB_STATE}" == "COMPLETED" ]]; then
                ...
                if check_convergence; then
                    echo "PolyChord completed successfully!"
                    exit 0
                else
                    echo "Job completed but PolyChord not converged. Resubmitting..."
                fi
...
echo "Check if PolyChord has converged or if there are persistent issues."
exit 1
